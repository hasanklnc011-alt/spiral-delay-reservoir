"""Cost-gated m25 final convergence check for the selected R=10 um bend."""
...
RUNS=HERE/"runs";ESTIMATE_RECORD=RUNS/"g3b-bend-r10-m25-estimate-v2.json";MEDIUM_FILE=RUNS/"g3b-bend-r10-dense_m20-result-v2.hdf5"
def build_case()->tuple[Any,str]:
    s=build_bend(radius_um=10.0,mesh=25.0,arc_points=385,run_time_s=1.3e-12);return s,hashlib.sha256(s.model_dump_json().encode()).hexdigest()
def estimate(*,approved:bool)->dict[str,Any]:
    if not approved:raise PermissionError("User credit approval required")
    s,d=build_case();name="p6_g3b_bend_r10_dense_m25_rt1p3";j=web.Job(simulation=s,task_name=name,folder_name="P6 Physical Validation",verbose=False);j.upload();c=float(j.estimate_cost(verbose=False));return {"name":"p6-g3b-bend-r10-m25-estimate-v2","source_sha256":hashlib.sha256(Path(__file__).read_bytes()).hexdigest(),"task_name":name,"task_id":str(j.task_id),"task_status":str(j.get_info().status),"serialized_simulation_sha256":d,"estimated_flexcredits":c,"solve_started":False,"within_limits":c<PER_SOURCE_LIMIT_FC and c<TOTAL_LIMIT_FC}
def execute(*,approved:bool)->dict[str,Any]:
    ...
    if not x["within_limits"] or x["solve_started"] or d!=x["serialized_simulation_sha256"]:raise RuntimeError("Locked m25 task is not eligible")
    p=RUNS/"g3b-bend-r10-dense_m25-result-v2.hdf5";j=web.Job(simulation=s,task_name=x["task_name"],folder_name="P6 Physical Validation",task_id_cached=x["task_id"],verbose=False);fine=_sparams(j.run(path=p));medium=_sparams(td.SimulationData.from_file(MEDIUM_FILE));center=6;ta=complex(**medium["s21"][center]);tb=complex(**fine["s21"][center]);conv={"insertion_loss_delta_db":abs(medium["transmission_db"][center]-fine["transmission_db"][center]),"phase_delta_deg":float(abs(np.angle(ta/tb,deg=True))),"fine_max_abs_energy_residual":fine["max_abs_energy_residual"],"fine_worst_reflection_db":fine["worst_reflection_db"]};conv["passed"]=conv["insertion_loss_delta_db"]<0.02 and conv["phase_delta_deg"]<0.5 and conv["fine_max_abs_energy_residual"]<0.01 and conv["fine_worst_reflection_db"]<-30
    return {"name":"p6-g3b-bend-r10-m25-convergence-result-v2","source_sha256":hashlib.sha256(Path(__file__).read_bytes()).hexdigest(),"task_id":x["task_id"],"task_status":str(j.get_info().status),"result_file":p.name,"result_sha256":hashlib.sha256(p.read_bytes()).hexdigest(),"actual_flexcredits":float(j.real_cost(verbose=False)),"convergence":conv,"g3b_passed":conv["passed"]}
def main():
