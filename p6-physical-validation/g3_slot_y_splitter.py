"""Cost-gated slot-opening adiabatic Y splitter with constant-width output rails."""
...
RUNS=HERE/"runs";ESTIMATE=RUNS/"g3c-slot-y-estimate-v1.json"
TAPER_UM=10.0;SLOT_OPEN_UM=40.0;OUTPUT_CENTER_UM=1.0;LEAD_UM=4.0
def poly(vertices,h):return td.PolySlab(vertices=vertices,slab_bounds=(-h/2,h/2),axis=2)
def build(mesh:float=15.0,run_time_s:float=4e-12)->td.Simulation:
    ...
    taper=[(x0,-w/2),(x0,w/2),(x_t,w),(x_t,-w)]
    xs=np.linspace(x_t,x1,401);u=(xs-x_t)/SLOT_OPEN_UM;s=.5-.5*np.cos(np.pi*u);g=final_gap*s;outer=w+g/2
    outer_vertices=list(zip(xs,-outer))+list(zip(xs[::-1],outer[::-1]));slot_vertices=[(x_t,0.0)]+list(zip(xs[1:],-g[1:]/2))+list(zip(xs[:0:-1],g[:0:-1]))
    structures=[td.Structure(geometry=td.Box(center=(x0-LEAD_UM/2,0,0),size=(LEAD_UM,w,h)),medium=si,name="input_lead"),td.Structure(geometry=poly(taper,h),medium=si,name="input_double_width_taper"),td.Structure(geometry=poly(outer_vertices,h),medium=si,name="outer_silicon"),td.Structure(geometry=poly(slot_vertices,h),medium=sio2,name="opening_silica_slot")]
...
def estimate(*,approved:bool)->dict[str,Any]:
    ...
    s=build();d=hashlib.sha256(s.model_dump_json().encode()).hexdigest();n="p6_g3c_slot_y_m15";j=web.Job(simulation=s,task_name=n,folder_name="P6 Physical Validation",verbose=False);j.upload();c=float(j.estimate_cost(verbose=False));return {"name":"p6-g3c-slot-y-estimate-v1","source_sha256":hashlib.sha256(Path(__file__).read_bytes()).hexdigest(),"task_name":n,"task_id":str(j.task_id),"task_status":str(j.get_info().status),"serialized_simulation_sha256":d,"simulation_cells":int(s.num_cells),"computational_grid_points":int(s.num_computational_grid_points),"estimated_flexcredits":c,"solve_started":False,"within_limits":c<PER_SOURCE_LIMIT_FC and c<TOTAL_LIMIT_FC}
def execute(*,approved:bool)->dict[str,Any]:
    ...
    p=RUNS/"g3c-slot-y_m15-result-v1.hdf5";j=web.Job(simulation=s,task_name=x["task_name"],folder_name="P6 Physical Validation",task_id_cached=x["task_id"],verbose=False);m=metrics(j.run(path=p));passed=m["center_excess_loss_db"]<=.2 and m["center_imbalance_db"]<=.5 and m["center_phase_mismatch_deg"]<=2 and m["worst_reflection_db"]<-30 and m["max_abs_energy_residual"]<.01;return {"name":"p6-g3c-slot-y-result-v1","source_sha256":hashlib.sha256(Path(__file__).read_bytes()).hexdigest(),"geometry":{"input_taper_um":TAPER_UM,"slot_open_um":SLOT_OPEN_UM,"output_center_um":OUTPUT_CENTER_UM},"task_id":x["task_id"],"task_status":str(j.get_info().status),"result_file":p.name,"result_sha256":hashlib.sha256(p.read_bytes()).hexdigest(),"actual_flexcredits":float(j.real_cost(verbose=False) or 0),"metrics":m,"pilot_passed":passed}
def main():
